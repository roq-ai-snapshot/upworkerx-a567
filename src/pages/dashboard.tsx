...
function DashboardPage() {
  ...
  const freelancerStat = useSWR<PaginatedInterface<FreelancerInterface>>(
    () => `/freelancers`,
    () => getFreelancers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Freelancers',
      result: freelancerStat?.data?.totalCount,
      isLoading: freelancerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Applications',
