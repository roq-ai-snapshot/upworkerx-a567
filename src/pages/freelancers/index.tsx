...
import { getFreelancers, deleteFreelancerById } from 'apiSdk/freelancers';
import { FreelancerInterface } from 'interfaces/freelancer';

type ColumnType = ColumnDef<FreelancerInterface, unknown>;

interface FreelancerListPageProps {
...
export function FreelancerListPage(props: FreelancerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getFreelancers({
        relations: ['user', 'application.count', 'booking.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['skills.contains', 'experience.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FreelancerInterface>>(
    () => `/freelancers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: FreelancerInterface) => {
    if (hasAccess('freelancer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/freelancers/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/freelancers/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('freelancer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/freelancers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('freelancer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="freelancer" />
        </Flex>

        {hasAccess('freelancer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/freelancers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
